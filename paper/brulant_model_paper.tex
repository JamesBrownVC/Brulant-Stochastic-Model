...
\title{%
  ...
  \large A 4-Factor Coupled Stochastic Differential System\\
  for Cryptocurrency Microstructure and Liquidation Cascades
}
...
% ============================================================================
\begin{abstract}
We present a novel four-factor stochastic differential equation (SDE) system
designed for the microstructure of cryptocurrency markets. The \textbf{Brulant Model}
couples spot price dynamics ($S_t$), stochastic volatility ($\sigma_t$),
an exhaustion memory process ($M_t$) governing a self-exciting jump intensity,
and a directional elasticity buffer ($B_t$) that determines jump direction
and provides intrinsic Euler--Maruyama drift correction. We derive analytical
properties of the system including the conditional jump intensity distribution,
the buffer's role as a mean-reversion anchor, and the interaction between
volatility compression and cascade probability. Calibration is performed via
Simulated Method of Moments (SMM) with $L_2$ regularization and differential
evolution. Out-of-sample validation on BTC/USDT 1-minute data demonstrates
a train loss of $\sim$2.0 and OOS loss of $\sim$3.8, with stable parameters
across walk-forward folds. We provide a complete digital option pricing
framework and demonstrate the model's superiority over standard 3-factor
jump-diffusion baselines.
\end{abstract}
...
We have presented the Brulant Model, a four-factor coupled SDE system
that captures the essential microstructure of cryptocurrency markets:
volatility compression leading to liquidation cascades, directional
jump selection via accumulated tension, and intrinsic discretization
drift correction.

Key contributions:
\begin{enumerate}[nosep]
  \item \textbf{The Sandpile Mechanic:} Self-exciting jump intensity
    driven by volatility compression and exhaustion memory.
  \item \textbf{The Elasticity Buffer:} A directional tension process
    that controls jump direction and provides intrinsic Euler--Maruyama
    drift correction.
  \item \textbf{Rigorous Validation:} Six-phase out-of-sample protocol
    demonstrating generalization with train loss $\sim$2.0 and OOS loss
    $\sim$3.8.
...
\begin{itemize}[nosep]
  \item Multi-asset extension: correlated buffer processes for portfolio
    pricing.
  \item Implied volatility surface inversion: from Brulant digital prices
    to an implied vol surface.
  \item Higher-order discretization: Milstein or stochastic Runge--Kutta
    schemes for reduced bias.
  \item Real-time calibration: online SMM with streaming Binance data.
  \item Regime detection: using the $(D_t, B_t)$ phase space as a
